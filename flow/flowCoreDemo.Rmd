...
```{r echo=TRUE}
fcsFiles <- dir("/home/rossco/Documents/I&IGeeks/flow/fcs", full.names = TRUE)
...
Now typically you might need to compensate your data. Lucky for me this data was compensated on the machine. But, if you were to compensate you would use the following method:

`compensate(gs, comp)`

Where `comp` is a matrix of spillover values.

Since my data is already compensated, I can access the compensation data as an attribute of the 'description' slot:
...
A simple gating strategy would be as follows:

```{r}
#Get our flowFrame
fr <- getData(gs[[1]], "root")
chnls <- c("FSC-A", "SSC-A")
tCells <- openCyto:::.flowClust.2d(fr, channels = chnls, K=3,
                                   target=c(70000, 50000),
                                   quantile=0.9)
